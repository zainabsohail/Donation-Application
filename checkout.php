...
<main role="main">
   ...
   <div class="album py-5 bg-light" style="padding-top:5rem; padding-bottom: 4rem;">
        <div class="container">
            <div class="row">
                <h2 style="text-align: center; font-family: 'Arvo',courier, serif;"> Check Out </h2>
                    <div class="product_section">
                        <?php
                        if(isset($_SESSION['order_id']) !=""){
                            $total_bill = 0;
                            $order_id = $_SESSION['order_id'];
                            $query = mysqli_query($conn, "delete from cart where product_id ='$_GET[product_id]' and order_id = '$order_id' ");
                               
                            $query = mysqli_query( $conn, "select * from clothes INNER JOIN cart ON cart.product_id=clothes.product_id WHERE cart.order_id= '$order_id'");
                            while ($row= mysqli_fetch_array($query))
                            {
                                $total_bill = $total_bill+$row['product_price'];
                        ?>
                            <div class="single_product text_center" style="display: inline-block; margin: 20px;">
                            <a href="checkout.php?product_id= <?php echo $row['product_id']; ?>" style="float: top; padding-left: 250px;">X</a>
                            <img src="images/<?php echo $row['clothes_img']; ?>"width="148px" height="223px"/>
                            <div class="card-body">
                                <p class="card-text"></p>
                            <div class="d-flex justify-content-between align-items-center">
                            <h4>Description : <?php echo $row['product_desc'];?></h4>
                            <h4>Status : <?php echo $row['product_status'];?></h4>
                            <h4>Quantity : <?php echo $row['product_qty'];?></h4>
                            <h4>Price : <?php echo $row['product_price'];?></h4>
                            </div>
                            </div>
                            </div>
                                <?php }  ?>
                            <div class="single_product text-center" style="display: block; font-size: 20px;text-align: center;">        
                                <?php echo "Bill<br> RS ".$total_bill ; ?>
                            </div>
                              <?php }  else { ?>
                        <div class="product_section">
